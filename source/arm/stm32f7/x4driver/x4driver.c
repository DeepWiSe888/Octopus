...
{
		return XEP_ERROR_X4DRIVER_OK;
//	if(mutex_lock(x4driver->lock.object) == 0)
//	{
//		return XEP_ERROR_X4DRIVER_OK;
//	}
//	else
...
static void mutex_give(X4Driver_t* x4driver)
{
 ...
 //	mutex_unlock(x4driver->lock.object);
}
...
uint32_t _update_normalization_variables(X4Driver_t* x4driver)
{
	uint8_t bytes_per_counter ;
	uint8_t iterations;
	uint16_t pulses_per_step;
	uint16_t dac_min ;
	uint16_t dac_max ;
	xtx4_dac_step_t dac_step = 0;
	xtx4_tx_center_frequency_t center;
    ...
    center = TX_CENTER_FREQUENCY_EU_7_290GHz;
...
int _x4driver_unpack_frame(X4Driver_t* x4driver,uint32_t* bins_data, uint32_t bins_data_size ,uint8_t * raw_data, uint32_t raw_data_length)
{
    ...
    mask = _get_mask(x4driver->bytes_per_counter);

    raw_data_index = 0;
    for ( i = 0; i<bins_data_size;i++)
    {
...
int _x4driver_unpack_and_normalize_downconverted_frame(X4Driver_t* x4driver,float* bins_data, uint32_t bins_data_size ,uint8_t * raw_data, uint32_t raw_data_length)
{
    ...
    nfactor = x4driver->normalization_nfactor * x4driver->filter_normalization_factor;
    q_data_start = bins_data_size/2;
    i_data_start = 0;
    raw_data_index = 0;
...
//        #pragma GCC diagnostic pop
        ...
        fbin = ((float32_t)bin_data*nfactor);
...
int _x4driver_unpack_and_normalize_frame(X4Driver_t* x4driver,float* bins_data, uint32_t bins_data_size ,uint8_t * raw_data, uint32_t raw_data_length)
{
    ...
    mask = _get_mask(x4driver->bytes_per_counter);
    nfactor = x4driver->normalization_nfactor;
    offset = x4driver->normalization_offset;
    raw_data_index = x4driver->frame_area_start_bin_offset * x4driver->bytes_per_counter;
    zero_frame = true;
...
int x4driver_verify_firmware(X4Driver_t* x4driver, const uint8_t * buffer, uint32_t size)
{
    ...
    read_back = 0x00;
    max_retries = 32;// FIFO depth is 8
    retries = 0;
...
double findmin(double in1, double in2)
{
		if(in1 > in2)
			return in2;
		else
...
double findmax(double in1, double in2)
{
		if(in1 > in2)
			return in1;
		else
...
{
		float32_t length = 0;
		float32_t X4DRIVER_MAX_BINS_RANGE_METERS = 0;
		uint32_t status = 0;
		int32_t NumberOfBinsPerFrame = 0;
		double NumberOfBinsDiscarded = 0;
		double DecimationFactor = 0;
		double FrameAreaStart = 0;
		double FrameAreaStop = 0;
		double FrameAreaOffset = 0;

		double dR_bin = 0;
		double R_frame_step = 0;
		double R_FrameStart_discarded = 0;	      
		double diff_RxTxWait = 0;
		uint8_t TxWait = 0;
		double RxWait = 0;
		uint8_t RxWait_8 = 0;
		double FrameStart_quantized	= 0 ;
		double diff_eps_meter = 0; 
		double bin_count_diff = 0;
		double diff = 0 ;
		uint32_t FrameAreaStart_quantized_ind = 0;
		uint32_t FrameAreaStop_quantized_ind = 0;
		double FrameAreaStart_quantized = 0;
		double FrameAreaStop_quantized = 0;
    ...
    // Set correct max bins, down-conversion requires 4 first bins to initialize filter
     X4DRIVER_MAX_BINS_RANGE_METERS = X4DRIVER_MAX_BINS_RANGE_METERS_RF;
    ...
    FrameAreaStop = end;
    FrameAreaOffset = x4driver->frame_area_offset_meters;
    ...
    diff_RxTxWait = floor( (FrameAreaStart - FrameAreaOffset) / R_frame_step );
    if (diff_RxTxWait < -1)
        diff_RxTxWait = -1;
    ...
    RxWait = (double)TxWait+diff_RxTxWait;
    RxWait_8 = (uint8_t)RxWait;
    ...
    FrameAreaStart = findmax(FrameAreaStart, FrameStart_quantized);		
    FrameAreaStop = findmin(FrameAreaStop, ((dR_bin * (NumberOfBinsPerFrame - NumberOfBinsDiscarded - 1)) + FrameStart_quantized));
    FrameAreaStop = findmax(FrameAreaStop, FrameAreaStart);

    // Find start bin index in the captured frame
    diff_eps_meter=1e-4; 
    bin_count_diff = (FrameAreaStart - FrameStart_quantized) / dR_bin;
    diff = bin_count_diff - ceil(bin_count_diff);
    ...
    if (x4driver->downconversion_enabled)
    {
        bytes_to_read = (x4driver->frame_area_end_ram_line - x4driver->frame_area_start_ram_line + 1) * 2 * x4driver->bytes_per_counter;
        x4driver->frame_read_size = bytes_to_read;
    }
    else
    {
        bytes_to_read = (x4driver->frame_area_end_ram_line - x4driver->frame_area_start_ram_line + 1) * 4 * x4driver->bytes_per_counter;
        x4driver->frame_read_size = bytes_to_read;
    }
...
int x4driver_set_frame_area_offset(X4Driver_t* x4driver, float32_t offset_in_meters)
{
    ...
    status = x4driver_set_frame_area(x4driver,x4driver->frame_area_start_requested, x4driver->frame_area_end_requested);
    return status;
}
 ...
 * @return Status of execution as defined in x4driver.h.
 */
char SPI1_Rx_Buf[1800];
int x4driver_read_frame_normalized(X4Driver_t* x4driver, uint32_t* frame_counter, float32_t* data, uint32_t length)
{
    uint32_t status = mutex_take(x4driver);
    if (status != XEP_ERROR_X4DRIVER_OK) return status;
    //read_raw_data
    status = x4driver_read_frame_bytes(x4driver, frame_counter, SPI1_Rx_Buf, x4driver->frame_read_size);
    //_x4driver_unpack_and_normalize_downconverted_frame(x4driver,data,length,SPI1_Rx_Buf,x4driver->frame_read_size);

    if (x4driver->downconversion_enabled == 0)
    {
        _x4driver_unpack_and_normalize_frame(x4driver,data,length,SPI1_Rx_Buf,x4driver->frame_read_size);
    }
    else
    {
        _x4driver_unpack_and_normalize_downconverted_frame(x4driver,data,length,SPI1_Rx_Buf + 2,x4driver->frame_read_size);
    }
...
int x4driver_read_frame_bytes(X4Driver_t* x4driver, uint32_t* frame_counter, uint8_t* data, uint32_t length)
{
    ...
    *frame_counter = x4driver->frame_counter;
    ...
    radar_data_addr = ADDR_SPI_RADAR_DATA_SPI_RE;
...
{
		uint32_t status = mutex_take(x4driver);
		uint8_t register_write_buffer[2];
		if (status != XEP_ERROR_X4DRIVER_OK) return status;
		address |= 0x80;
		register_write_buffer[0] = address;
		register_write_buffer[1] = value;
...
int x4driver_get_spi_register(X4Driver_t* x4driver,uint8_t address, uint8_t * value)
{
    ...
    status = x4driver->callbacks.spi_write_read(0, &register_write_buffer, 1,&register_read_buffer,1);
    mutex_give(x4driver);
...
int _x4driver_set_internal_register(X4Driver_t* x4driver, uint8_t address, uint8_t write_value, uint8_t command)
{
    ...
    retries_max = PIF_COMMAND_MAX_RETRIES;
...
{
	uint8_t OSC_CTRL_REGISTER ;
	uint32_t ossc_lock_attemps ;
	uint8_t common_pll_status_register_value ;
    uint8_t osc_lock_bit ;
    uint32_t has_lock ;
	uint32_t i;
    uint32_t status = mutex_take(x4driver);
    if (status != XEP_ERROR_X4DRIVER_OK) return status;
    OSC_CTRL_REGISTER = 0x00;
    ossc_lock_attemps = OSC_LOCK_ATTEMPS_MAX;
    ...
    common_pll_status_register_value = 0x00;
    osc_lock_bit = 1 << 6;
    has_lock = 0;
    for ( i = 0; i < ossc_lock_attemps; i++)
    {
...
int _x4driver_enable_dvdd_tx(X4Driver_t* x4driver)
{
    ...
    for ( i = 0; i <16;i++)
    {
        ...
        //power not good
        x4driver_get_pif_register(x4driver, ADDR_PIF_DVDD_TX_CTRL_RW,&dvdd_tx_ctrl_value);
        trim_value = dvdd_tx_ctrl_value & 0x1f;
        //trim value 0
...
int _x4driver_enable_dvdd_rx(X4Driver_t* x4driver)
{
	...
	unsigned char i;
	uint8_t trim_value;
	uint8_t dvdd_rx_ctrl_value;
    uint32_t status = mutex_take(x4driver);
    if (status != XEP_ERROR_X4DRIVER_OK) return status;
    //set dvdd_testmode 5
    x4driver_set_pif_register(x4driver,ADDR_PIF_APC_DVDD_TESTMODE_RW,5);
    dvdd_rx_ctrl_value = 0x00;
    ...
    for ( i = 0; i <16;i++)
    {
        ...
        //power not good
        x4driver_get_pif_register(x4driver, ADDR_PIF_DVDD_RX_CTRL_RW,&dvdd_rx_ctrl_value);
        trim_value = dvdd_rx_ctrl_value & 0x1f;
        //trim value 0
...
int _x4driver_enable_avdd_tx(X4Driver_t* x4driver)
{
    ...
    for ( i = 0; i <16;i++)
    {
        ...
        //power not good
        x4driver_get_pif_register(x4driver, ADDR_PIF_AVDD_TX_CTRL_RW,&avdd_tx_ctrl_value);
        trim_value = avdd_tx_ctrl_value & 0x1f;
        //trim value 0
...
int _x4driver_enable_avdd_rx(X4Driver_t* x4driver)
{
	...
	uint8_t trim_value ;
	uint8_t ldo_status_2_value;
	uint8_t avdd_rx_ctrl_value;
    if (status != XEP_ERROR_X4DRIVER_OK) return status;
    //set dvdd_testmode 5
    x4driver_set_pif_register(x4driver,ADDR_PIF_APC_DVDD_TESTMODE_RW,5);
     avdd_rx_ctrl_value = 0x00;
    ...
    for ( i = 0; i <16;i++)
    {
        ...
        //power not good
        x4driver_get_pif_register(x4driver, ADDR_PIF_AVDD_RX_CTRL_RW,&avdd_rx_ctrl_value);
        trim_value = avdd_rx_ctrl_value & 0x1f;
        //trim value 0
...
{
	uint32_t status;
	uint8_t rx_mframes_val;
	uint32_t nrangebins ;
	uint8_t rx_counter_num_bytes;
    ...
    rx_mframes_val = cycles * (96/12);
    x4driver_set_pif_register(x4driver,ADDR_PIF_RX_MFRAMES_RW,rx_mframes_val);
    nrangebins = cycles*96;
    rx_counter_num_bytes = 0x00;
...
{ 
    ...
    _segment_value =  (reg_value & mask) >>start_bit;
...
{
	uint16_t _dac_min ;
    uint8_t _dac_min_lsb;
    uint8_t _dac_min_msb;
	 uint32_t status;
    if (dac_min > 0x07ff)
        return XEP_ERROR_X4DRIVER_NOK;
    status = mutex_take(x4driver);
    if (status != XEP_ERROR_X4DRIVER_OK) return status;
    _dac_min = dac_min;
    _dac_min_lsb = (uint8_t)(_dac_min & 0x0007);
    _dac_min_msb = (uint8_t)((_dac_min >> 3) & 0x00ff);
...
{
	uint16_t _dac_max ;
    uint8_t _dac_max_lsb ;
    uint8_t _dac_max_msb ;
	uint32_t status	;
    if (dac_max > 0x07ff)
        return XEP_ERROR_X4DRIVER_NOK;
    ...
    if (status != XEP_ERROR_X4DRIVER_OK) return status;
     _dac_max = dac_max;
     _dac_max_lsb = (uint8_t)(_dac_max & 0x0007);
     _dac_max_msb = (uint8_t)((_dac_max >> 3) & 0x00ff);
...
int x4driver_set_fps(X4Driver_t* x4driver, float32_t fps)
{
    ...
    else if (x4driver->trigger_mode == SWEEP_TRIGGER_X4)
    {
        ...
        if (fps > FLT_EPSILON)
        {
            ...
            fps_uint = (uint32_t)fps;
            if (fps_uint < 35)
            {
                x4driver->configured_fps = measured_fps_from_x4_timer_1_34hz[fps_uint-1];
            }
             fps_lsb = (uint8_t)(fps_uint & 0x000000ff);
             fps_msb = (uint8_t)((fps_uint & 0x0000ff00)>>8);
...
int _x4driver_set_rx_ram_first_line(X4Driver_t* x4driver, uint32_t first_line)
{
	  uint32_t rx_ram_line_first ;
    uint8_t rx_ram_line_first_lsb ;
    uint8_t rx_ram_line_first_msb;
    uint32_t status = mutex_take(x4driver);
    if (status != XEP_ERROR_X4DRIVER_OK) return status;

    rx_ram_line_first = first_line;
    rx_ram_line_first_lsb = (uint8_t)rx_ram_line_first & 0x00000001;
    rx_ram_line_first_msb = (uint8_t)(rx_ram_line_first >>1) & 0x000000ff;
...
int _x4driver_set_rx_ram_last_line(X4Driver_t* x4driver,uint32_t last_line)
{
    uint32_t rx_ram_line_last ;
    uint8_t rx_ram_line_last_lsb ;
    uint8_t rx_ram_line_last_msb ;
    uint32_t status = mutex_take(x4driver);
    if (status != XEP_ERROR_X4DRIVER_OK) return status;

    rx_ram_line_last = last_line;
    rx_ram_line_last_lsb = (uint8_t)rx_ram_line_last & 0x00000001;
    rx_ram_line_last_msb = (uint8_t)(rx_ram_line_last >>1) & 0x000000ff;
...
int x4driver_get_sweep_time(X4Driver_t* x4driver, float32_t * sweep_time)
{
    uint16_t dac_min =0;
    uint16_t dac_max =0;
	uint8_t dac_step_val; 
	float32_t total_steps;
	float32_t dac_steps	;
	uint8_t iterations;
	uint8_t prf_div;
    float32_t prf;
    float32_t total_time ;
	uint16_t pulses_per_step;
    xtx4_dac_step_t dac_step =DAC_STEP_1;
    x4driver_get_dac_min(x4driver,&dac_min);
    x4driver_get_dac_max(x4driver,&dac_max);
    x4driver_get_dac_step(x4driver,&dac_step);
    dac_step_val =  1 << dac_step;
    ...
    dac_steps = (dac_max - dac_min)/dac_step_val;
    total_steps = dac_steps*pulses_per_step*iterations;
    prf_div = 0;
    x4driver_get_prf_div(x4driver,&prf_div);
    prf = 243e6/prf_div;
    total_time = (1/prf)*total_steps;
...
int x4driver_check_configuration(X4Driver_t* x4driver)
{
    ...
    pulses_per_step = 0;
    x4driver_get_pulses_per_step(x4driver,&pulses_per_step);
    dac_range = ((dac_max-dac_min)/dac_step_val)-1;
    max_counter = 16777215;
    ...
    trx_backend_clk_totaldiv = trx_backend_clk_div*trx_backend_clk_prescale*pllauxclk_sel;
    trx_dac_settle_clog2 = 0;
...
int x4driver_set_dac_auto(X4Driver_t* x4driver,uint32_t daclength)
{
		...
		float32_t org_fps = 0;
		uint32_t fc = 0;
		xtx4_sweep_trigger_control_mode_t org_tm ;
		uint32_t i;
    if (daclength > 2047)
    {
        return XEP_ERROR_X4DRIVER_NOK;
...
int x4driver_set_frame_trigger_period(X4Driver_t* x4driver, uint32_t period)
{
    ...
    {
        period_seconds = (1.0/10000)*period;
