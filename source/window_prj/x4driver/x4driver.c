...
#define x4_TAG                     "x4driver"            //打印的x4_TAG
const float32_t measured_fps_from_x4_timer_1_34hz[] = {
    1.004108124950404, 2.0080154538542674, 3.0120231300774165, 4.0152279344517687,
    5.0185332341280828, 6.0240460751902258, 7.0273512120334267, 8.0272459942058507,
    9.030651670395633, 10.032052556008651, 11.035257553778095, 12.040868513866339,
    13.041667871447936, 14.044873167198913, 15.05559814087251, 16.04166747731395,
    17.049378771457828, 18.061302650200414, 19.055187571769331, 20.044079750069113,
    21.052587257711036, 22.070514637123811, 23.085253644961522, 24.081739235799915,
    25.042606665713322, 26.083338304556776, 27.067615812132296, 28.050517228370968,
    29.107493859414152, 30.066123144589028, 31.090047130967591, 32.083338705938836,
    33.033175120617237, 34.040966553477801};
...
int _x4driver_unpack_and_normalize_downconverted_frame(X4Driver_t* x4driver,float* bins_data, uint32_t bins_data_size ,uint8_t * raw_data, uint32_t raw_data_length)
{
    ...
    uint32_t raw_data_index = 0;
    //uint32_t sbits = (x4driver->bytes_per_counter << 3) - 1;
    ...
    //signbit_mask = 1ULL << sbits;
    //convert_mask = (1ULL << (sbits+1)) - 1;
 ...
 * @return Status of execution as defined in x4driver.h
 */
int x4driver_set_predefined(X4Driver_t* d, X4DriverCallbacks_t* x4driver_callbacks, X4DriverLock_t *lock,X4DriverTimer_t *timer, X4DriverTimer_t *timer_action, void* user_reference)
{
...
{
    printf( "x4driver_upload_firmware_default");
    uint32_t status = mutex_take(x4driver);
    if (status != XEP_ERROR_X4DRIVER_OK)
        return status;
...
int x4driver_set_frame_area(X4Driver_t* x4driver, float32_t start, float32_t end)
{
    ...
    if (x4driver->downconversion_enabled)
    {
        x4driver->frame_read_size = (x4driver->frame_area_end_ram_line - x4driver->frame_area_start_ram_line + 1) * 2 * x4driver->bytes_per_counter;
    }
    else
    {
        x4driver->frame_read_size = (x4driver->frame_area_end_ram_line - x4driver->frame_area_start_ram_line + 1) * 4 * x4driver->bytes_per_counter;
    }
 ...
 */
#include <sys/time.h>
int sub_timeval(struct timeval* tv1, struct timeval* tv2) {
    int t1 = 0, t2 = 0;
    ...
    if (tv2->tv_usec >= tv1->tv_usec)
        ...
        t1--;
        t2 = 1000000 + tv2->tv_usec - tv1->tv_usec;
    }
    return t1*1000000+t2;
}
...
int x4driver_read_frame_bytes(X4Driver_t* x4driver, uint32_t* frame_counter, uint8_t* data, uint32_t length)
{
    ...
    *frame_counter = x4driver->frame_counter;
    ...
    status = x4driver->callbacks.spi_write_read(x4driver->user_reference, &radar_data_addr, 1, data, /*x4driver->frame_read_size*/length);
...
int x4driver_write_to_spi_register(X4Driver_t* x4driver, uint8_t address, const uint8_t* values, uint32_t length)
{
    ...
    if ((length+1) >= x4driver->spi_buffer_size)
        return XEP_ERROR_X4DRIVER_BUFFER_TO_SMALL;
    address |= 0x80;
...
int _x4driver_enable_dvdd_rx(X4Driver_t* x4driver)
{
    ...
    if (status != XEP_ERROR_X4DRIVER_OK) return status;
    //set dvdd_testmode 5
    x4driver_set_pif_register(x4driver,ADDR_PIF_APC_DVDD_TESTMODE_RW,5); // all
...
int x4driver_init(X4Driver_t* x4driver)
{
    ...
    x4driver->callbacks.wait_us(50000);
    SPI_open();
#endif
   ...
   // x4driver_get_spi_register(x4driver, ADDR_SPI_FORCE_ONE_R, &force_one);
    ...
    if (status != XEP_ERROR_X4DRIVER_OK)
    {
        x4driver->initialized = 0;
        mutex_give(x4driver);
        printf( "Upload firmware error.\n");
        return status;
    }
    printf( "Upload firmware finished.");
    ...
    if (status != XEP_ERROR_X4DRIVER_OK)
    {
        x4driver->initialized = 0;
        mutex_give(x4driver);
        printf( "update_normalization_variables");
        return status;
    }
    printf( "update_normalization_variables finished");
    ...
    if (status != XEP_ERROR_X4DRIVER_OK)
    {
        x4driver->initialized = 0;
        mutex_give(x4driver);
        printf( "x4driver_set_frame_area");
        return status;
    }
    printf( "Set_frame_area");
    ...
    if (status != XEP_ERROR_X4DRIVER_OK)
    {
        x4driver->initialized = 0;
        mutex_give(x4driver);
        printf( "x4driver_set_downconversion 0");
        return status;
    }
    printf( "Set_downconversion");
...
void x4driver_enable_dataready_isr(X4Driver_t* x4driver)
{
    x4driver->callbacks.enable_data_ready_isr(x4driver->user_reference, 1);
}
